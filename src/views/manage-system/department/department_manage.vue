...
const address = ref({
  title: props.nameKCN,
  details: [{name: "Quản lý phòng ban", to: `${props.slug}ManageDepartmentPage`}],
});

const getData = async () => {
  const dept = await http.get
  (`/${props.slug}/department/?page=${option.currPage}&search=${option.searchVal}&manager=${option.managerVal}&start_date=${option.dateStart ? option.dateStart : ''}&end_date=${option.dateEnd ? option.dateEnd : ''}`);
  departmentData.value = dept.data.results;
...
function deleteData(id) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      const response = await http.delete(`/${props.slug}/department/${id}/`);
...
const viewDetail = async (item) => {
  isDetail.value = true;
  const deptDt = await http.get(`/${props.slug}/department/${item}/`);
...
onMounted(async () => {
  await getData();
  // departmentToGetManage.value = departmentData.value;
});
  ...
  <Layout :address="address" :items="address.details" chooseMenu="phong-ban" :slug="props.slug">
    <div class="manage-database">
      <FolderHeaderBar>
        <template #add_button>
          <b-col xl="2">
            <router-link type="button" class="btn btn-light" :to="{ name: `${props.slug}AddDepartmentPage` }">
    ...
    <div class="page-main__content" ref="containerContent">
      <b-card no-body class="mb-2">
        <div data-simplebar class="card-body__custom--scroll">
          <b-card-body class="card-body__custom--height" v-if="departmentData.length">
            <div class="table-responsive">
              <table class="table align-middle table-striped table-bordered">
                ...
                <tbody v-for="(item,index) in departmentData" :key="index">
                <tr class="odd dt-hasChild" :class="{ parent: showMoreItems[index] }">
                  ...
                  <td>{{ item.manager ? `${item.manager.first_name} ${item.manager.last_name}` : '' }}</td>
                    ...
                    <ul class="list-inline hstack gap-3 mb-0 d-flex justify-content-center">
                      <li class="list-inline-item">
                        <div class="link-info fs-15 cursor-pointer" v-b-tooltip.hover title="Xem chi tiết"
                             @click="viewDetail(item.short_name)">
                          <i class="ri-eye-line"></i>
                        </div>
                      </li>
                      <li class="list-inline-item">
                        <router-link :to="{ name: `${props.slug}UpdateDepartmentPage`, params: {id: item.short_name} }"
                      ...
                      <li class="list-inline-item">
                        <div class="link-danger fs-15 cursor-pointer" @click="deleteData(item.short_name)"
                             v-b-tooltip.hover
                             title="Xóa">
                  ...
                  <td class="child" colspan="5">
                    <ul data-dtr-index="0" class="dtr-details">
                      <li
                        ...
                        data-dt-column="5"
                      >
                        <span class="dtr-title fw-semibold">Ngày cập nhật</span>
                        <span class="dtr-data">
...
<!--                      <li
      ...
      <b-modal v-model="isDetail" hide-footer body-class="p-0" header-class="p-3"
               class="v-modal-custom" title="Chi tiết phòng ban" content-class="border-0 overflow-hidden" centered>
        <div class="p-2">
          <div class="table-responsive">
