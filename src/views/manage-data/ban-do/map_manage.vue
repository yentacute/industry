...
const getAPITableMap = () => {
  ...
  API()
    .get(
      `${props.slug}/map?page=${PaginationData.currentPage}&q=${feature.search}`
    )
    .then((res) => {
...
const address = ref({
  title: props.nameKCN,
  details: [{ name: "Quản lý bản đồ", to: `${props.slug}ManageMapPage` }],
});
...
const handleKeySearch = ($e) => {
  ...
  timeout = setTimeout(() => {
    API()
      .get(`${props.slug}/map/?q=${$e.target.value}`)
      .then((res) => {
        PaginationData.pageSize = res.total_pages;
  ...
  >
    <div class="d-flex h-100 justify-content-between flex-column">
      <FolderHeaderBar style="margin-bottom: 12px">
        <template #add_button>
          <b-col xl="2">
            <router-link :to="{ name: `${props.slug}ManageAddMap` }">
              ...
              <tbody>
                <tr v-for="(item, index) in dataMap" :key="index">
                  ...
                  <td :title="item.description">
                    {{ limitText(item.description, 300) }}
                  </td>
                  <td class="text-center">
                          ...
                          <ul class="dropdown-menu dropdown-menu-end">
                            <li>
                              <router-link
                                class="dropdown-item"
                                :to="{
                                  name: `${props.slug}MapPublicDetails`,
                                  params: { mapId: item.id },
                                }"
                              >
                                <i
                            ...
                            <li @click="handleClickDeleteMap(item)">
                              <b-link class="dropdown-item text-danger">
                                <i
                                  class="ri-delete-bin-fill align-bottom me-2"
                                ></i>
                                Xóa
                              </b-link>
                            </li>
