<template>
    <ModalCommon title="Thêm model" size="modal-xl" id-name="AddModelForm">
        <Form @submit="onSubmit" :initial-values="model"
            :validation-schema="schema">
            <div class="row align-items-center mb-3">
                <div class="col-2">
                    <Field name="modelname" type="text" class="form-control mb-2" v-model="model.modelname" placeholder="Tên model"/>
                    <ErrorMessage name="modelname" as="div" class="error-message"/>
                </div>
                <div class="col-2">
                    <Field name="name_display" type="text" class="form-control mb-2" v-model="model.name_display" placeholder="Tên hiển thị"/>
                    <ErrorMessage name="name_display" as="div" class="error-message"/>
                </div>
                <div class="col-auto">
                    <label for="use_uuid">use_uuid</label>
                    <input name="use_uuid" type="checkbox" class="form-check-input mx-2" v-model="model.use_uuid"/>
                </div>
                <div class="col-auto">
                    <label for="use_object3d">use_object3d</label>
                    <input name="use_object3d" type="checkbox" class="form-check-input mx-2" v-model="model.use_object3d"/>
                </div>
                <div class="col-3">
...
<script setup>
  ...
  //define model variable like define in option data data() {...}
                        ...
                        name: yup.string()
                                .when('geom', {
                                    ...
                                    then:() =>  yup.string()
                                        ...
                                        .matches(/^[a-zA-Z ]*$/, 'Tên model không bao gồm số, kỹ tự đặc biệt'),
    ...
    //set folder value every time value change
