<script setup>
import { inject, onMounted, onUnmounted, ref } from "vue";

const data_map = inject("map");
const methods_map = inject("draw-map");

const listPoint = ref([]);
const listLocation = ref([]);
const distance = ref(0);

const addPoint = (point) => {
  if (!point.lat || !point.lng) return;
  listLocation.value = [...listLocation.value, point];
  listPoint.value = [...listPoint.value, [point.lng, point.lat]];
};

const handleWindow = () => {
  if (!data_map.location.value) return;
  addPoint(data_map.location.value);
  methods_map.create_polyline("#0377fc", "solid", [...listPoint.value]);
  methods_map.MannyMakerMap(
    listLocation.value,
    require("@/assets/images/maker/dot_blue.png")
  );
  let Measure = new map4d.Measure(listPoint.value);
  distance.value = Math.round(100 * Measure.length) / 100;
};

onMounted(() => {
  data_map.location.value = null;
  window.addEventListener("click", handleWindow);
});

onUnmounted(() => {
  window.removeEventListener("click", handleWindow);
});
</script>

<template>
  <div @click.stop class="calculation-custom">
    <span
      >Chiều dài: <b>{{ `${distance} m` }}</b></span
    >
  </div>
</template>

<style scoped>
.calculation-custom {
  position: fixed;
  right: 60px;
  bottom: 10px;
  border-radius: 4px;
  background-color: #ccc;
  padding: 10px;
  display: flex;
  justify-content: center;
  align-items: center;
}

@media only screen and (max-width: 575px) {
  .calculation-custom {
    bottom: auto;
    top: 125px;
  }
}
</style>
